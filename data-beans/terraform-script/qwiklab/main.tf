...
#    command = <<-EOT
#      google-cloud-sdk/bin/gsutil cp -r gcp-public-demos/data-beans/assets/roaster_sensor/* gs://data_beans_${data.google_project.project.number}/roaster
#      google-cloud-sdk/bin/gsutil cp -r gcp-public-demos/data-beans/assets/visual_inspection/* gs://data_beans_${data.google_project.project.number}/visual_inspection
#      google-cloud-sdk/bin/gsutil cp -r gcp-public-demos/data-beans/assets/weather/* gs://data_beans_${data.google_project.project.number}/weather
#      google-cloud-sdk/bin/gsutil cp -r gcp-public-demos/data-beans/assets/claims/* gs://data_beans_${data.google_project.project.number}/claims
#      google-cloud-sdk/bin/gsutil cp -r gcp-public-demos/data-beans/assets/orders/* gs://data_beans_${data.google_project.project.number}/orders
#      EOT
#  }
#  depends_on = [
...
### New version of sub folder creation --------

##Create sub folders - claims
resource "google_storage_bucket_object" "claims_folder" {
  name          = "claims/"
  #content       = "Claims data"
  source        = "${path.module}/gcp-public-demos/data-beans/assets/claims/*"
  bucket        = "${google_storage_bucket.data_beans_bucket.name}"
  depends_on = [
  google_storage_bucket.data_beans_bucket,
  null_resource.get_from_github
  ]
}
...
resource "google_storage_bucket_object" "orders_folder" {
  name          = "orders/"
  #content       = "orders data"
  source        = "${path.module}/gcp-public-demos/data-beans/assets/orders/*"
  bucket        = "${google_storage_bucket.data_beans_bucket.name}"
  depends_on = [
  google_storage_bucket.data_beans_bucket,
  null_resource.get_from_github
...
resource "google_storage_bucket_object" "roaster_folder" {
  name          = "roaster/"
  #content       = "Roaster sensor data"
  source        =  "${path.module}/gcp-public-demos/data-beans/assets/roaster_sensor/*"
  bucket        = "${google_storage_bucket.data_beans_bucket.name}"
  depends_on = [
  google_storage_bucket.data_beans_bucket,
  null_resource.get_from_github
...
resource "google_storage_bucket_object" "visual_inspection_folder" {
  ...
  #content       = "Visual inspection data"
  source        = "${path.module}/gcp-public-demos/data-beans/assets/visual_inspection/*"
  bucket        = "${google_storage_bucket.data_beans_bucket.name}"
  depends_on = [
  google_storage_bucket.data_beans_bucket,
  null_resource.get_from_github
...
resource "google_storage_bucket_object" "weather_folder" {
  ...
  source        = "${path.module}/gcp-public-demos/data-beans/assets/weather/*"
  bucket        = "${google_storage_bucket.data_beans_bucket.name}"
  depends_on = [
  google_storage_bucket.data_beans_bucket,
  null_resource.get_from_github
...
resource "google_bigquery_table" "claims" {
   ...
   # }
   #}
   depends_on = [google_storage_bucket_object.claims_folder]
 }

##load claim data into native table
resource "null_resource" "load_claim_data" {
  provisioner "local-exec" {
    command =  "google-cloud-sdk/bin/bq --location=${var.gcp_region} load --autodetect --skip_leading_rows=1 --source_format=CSV ${google_bigquery_dataset.data_beans.dataset_id}.claims gs://data_beans_${data.google_project.project.number}/claims/databeans_claims.csv"
  }
...
module "cli" {
  ...
  create_cmd_entrypoint    = "bq --location=${var.gcp_region} load --autodetect --skip_leading_rows=1 --source_format=CSV ${google_bigquery_dataset.data_beans.dataset_id}.claims gs://data_beans_${data.google_project.project.number}/claims/databeans_claims.csv"
...
resource "google_bigquery_table" "orders" {
   ...
   depends_on = [google_project_iam_member.bucket, google_storage_bucket_object.orders_folder]
 }
